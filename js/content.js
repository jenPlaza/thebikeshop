  ...
  for (var i = 0; i < twentyFiveWidth.length; i++) {
    for (i = 0; i < dFeatures.products.length; i++) {
      ...
      if (dFeatures.products[i].salePrice === '') {
        ...
        data +=
          '<p>$' +
          dFeatures.products[i].salePrice +
          ' <strong><em>$' +
          dFeatures.products[i].price +
          '</em></strong></p>';
      }
      data += '</article>';
    }
...
function memberBenefits(dPlans) {
  ...
  dataPlan +=
    '<input type="email" placeholder="Confirm Email" name="email"><br /><br />';
  ...
  dataPlan +=
    '<input type="text" placeholder="Enter Birthdate" name="birthdate"><br /><br />';

  dataPlan += '<input id="radio" type="radio" name="radio">';
  dataPlan +=
    '<label for="radio"><b>Yes! Please send periodic information on product, services, and events</b></label><br /><br />';
  dataPlan += '<section id="thankYou">';
  dataPlan +=
    '<h2>Thankyou for trusting us and helping us grow into a huge success</h2><br /><br />';
  dataPlan += '</section><br />';
...
function clubEvents(dEvents) {
  ...
  var imageArray1 = [
    'images/cyclingRace.png',
    'images/bikeTrail1.png',
    'images/bikeTrail2.png',
    'images/cyclingRace2.png',
    'images/biketrail.jpg',
    'images/bikeTrail3.png',
  ];
...
function shopProducts(dShopP) {
  ...
  for (var i = 0; i < dShopP.products.length; i++) {
    ...
    dataSProducts += '<h5>' + dShopP.products[i].title + '</h5>';
    dataSProducts += '<p><em>' + dShopP.products[i].description + '</em></p>';
    dataSProducts +=
      '<h4>$' +
      dShopP.products[i].salePrice +
      '  ' +
      dShopP.products[i].price +
      '</h4>';
    dataSProducts += '<ul>';
    dataSProducts +=
      '<li><a href="#"><img src="images/zoom-in-2.png" alt="zoom-in icon" ></a></li>';
    dataSProducts +=
      '<li> <a href="#"><img src="images/shopping-cart.png" alt="shopping-cart icon" ></a></li>';
    dataSProducts +=
      '<li><a href="#"><img src="images/heart.png" alt="favorite icon" ></a></li>';
    dataSProducts += '</ul>';
